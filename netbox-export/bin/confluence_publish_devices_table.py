#!/usr/bin/env python3
"""Publish the NetBox devices CSV as a Confluence table."""
...
SELECTED_COLUMNS = ["Name", "Status", "Role", "IP Address", "OOB IP"]
...
def build_table_html(header: Sequence[str], rows: Iterable[Sequence[str]]) -> str:
    ...
    for row in rows:
        ...
        # Pad short rows to header length to avoid malformed table
...
def main() -> int:
    ...
    parser.add_argument(
        "--page-id",
        default=os.getenv("CONFLUENCE_DEVICES_PAGE_ID")
        or os.getenv("CONFLUENCE_CMDB_PAGE_ID")
        or os.getenv("CONFLUENCE_PAGE_ID"),
        help="Target Confluence page ID",
    )
    parser.add_argument(
        "--heading",
        default="NetBox Devices Export",
    ...
    parser.add_argument(
        "--message",
        default="Updated NetBox devices table",
    ...
    if not args.page_id:
        print("[red]Missing page ID[/red]. Supply --page-id or set CONFLUENCE_DEVICES_PAGE_ID/CONFLUENCE_CMDB_PAGE_ID.")
        return 2
    ...
    print("[green]Confluence page updated with NetBox devices table[/green]")
    return 0
