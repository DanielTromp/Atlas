...
def sync(full: bool, spaces: tuple[str], ancestor_id: str | None, verbose: bool):
    ...
    async def run():
        # Import here to avoid circular imports and delays
        ...
        # Note: In real script we might read .env here or let settings do it
        ...
        sync_engine = ConfluenceSyncEngine(
            confluence, db, chunker, embeddings, settings
        )
        
        space_list = list(spaces) if spaces else None
        
        if full:
            click.echo(f"Starting full sync for spaces: {space_list or 'all configured'}")
            click.echo(f"Active Label Filter: {settings.watched_labels}")
            await sync_engine.full_sync(space_list, ancestor_id=ancestor_id)
        else:
            click.echo(f"Starting incremental sync for spaces: {space_list or 'all configured'}")
