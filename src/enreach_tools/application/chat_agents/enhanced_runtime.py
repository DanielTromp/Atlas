...
from enreach_tools.agents import (
    DOMAIN_SPECS,
    AgentContext,
    RouterDecision,
    build_router_agent,
...
from enreach_tools.application.chat_agents.callbacks import (
    TokenUsageCallback,
    merge_token_usage,
)
from enreach_tools.application.chat_agents.runtime import AgentRunResult, AgentRuntimeError
from enreach_tools.infrastructure.logging import get_logger
from enreach_tools.infrastructure.openai_client import get_enhanced_openai_client
from enreach_tools.infrastructure.queues.chat_queue import (
    RequestPriority,
    get_chat_queue,
    queue_chat_request,
)
    ...
    async def _build_enhanced_llm(self, temperature_override: float | None = None) -> Any:
        ...
        if self._provider in {"openai", "openrouter"}:
            ...
            if self._provider == "openrouter":
                ...
                title = os.getenv("OPENROUTER_TITLE", "Enreach Tools").strip()
                if referer:
