...
from enreach_tools.domain.repositories import ChatSessionRepository
...
def create_chat_history_service(session: Session, repo_factory=None) -> DefaultChatHistoryService:
    if repo_factory is None:
        from enreach_tools.infrastructure.db.repositories import SqlAlchemyChatSessionRepository
