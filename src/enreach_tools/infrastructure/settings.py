...
class Settings:
    ...
    def from_env(cls) -> Settings:
        load_env()
        return cls(
            log_level=os.getenv("ENREACH_LOG_LEVEL", "INFO"),
            structured_logging=os.getenv("ENREACH_LOG_STRUCTURED", "").strip().lower() in {"1", "true", "yes", "on"},
            database_url=os.getenv("ENREACH_DB_URL", ""),
        )
