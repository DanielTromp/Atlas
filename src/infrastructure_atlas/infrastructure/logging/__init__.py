...
_LOG_CONTEXT: ContextVar[dict[str, Any] | None] = ContextVar("atlas_log_context", default=None)
...
def _install_file_handler(level: int) -> None:
    ...
    handler = RotatingFileHandler(
        target_str,
        maxBytes=_resolve_int_env("ATLAS_LOG_MAX_BYTES", 5 * 1024 * 1024),
        backupCount=_resolve_int_env("ATLAS_LOG_BACKUP_COUNT", 5),
    )
...
def _resolve_log_path() -> Path:
    directory = Path(os.getenv("ATLAS_LOG_DIR", "logs")).expanduser()
    directory.mkdir(parents=True, exist_ok=True)
    filename = os.getenv("ATLAS_LOG_FILE", "atlas.log").strip() or "atlas.log"
...
def _resolve_level(level: str | int | None) -> int:
    ...
    env_level = os.getenv("ATLAS_LOG_LEVEL", os.getenv("LOG_LEVEL", "INFO"))
...
def _resolve_structured(structured: bool | None) -> bool:
    if structured is not None:
        return structured
    raw = os.getenv("ATLAS_LOG_STRUCTURED", "").strip().lower()
