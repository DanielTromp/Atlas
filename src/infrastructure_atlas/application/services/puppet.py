...
class MongoDBPuppetService:
    """MongoDB-based Puppet configuration and inventory management service."""
    ...
    def update_config(
        ...
        *,
        ...
        if name is not None:
            ...
            if not normalised:
                raise ValueError("Name cannot be empty")
            update_data["name"] = normalised

        if remote_url is not None:
            update_data["remote_url"] = _normalise_remote_url(remote_url)

        if branch is not None:
            update_data["branch"] = _normalise_branch(branch)
        ...
        if ssh_key_path is not None:
            ...
            if ssh_key_path:
                ...
                secret_name = f"puppet:{config_id}:ssh_key_path"
                store.set(secret_name, ssh_key_path)
                update_data["ssh_key_secret"] = secret_name
            else:
                # Clear SSH key
                if config.ssh_key_secret:
                    store = require_secret_store()
                    store.delete(config.ssh_key_secret)
                update_data["ssh_key_secret"] = None
...
PuppetServiceProtocol = PuppetService | MongoDBPuppetService


def create_puppet_service(session: Any = None) -> PuppetServiceProtocol:
    ...
    if backend == "mongodb":
        ...
        from infrastructure_atlas.infrastructure.mongodb.repositories import MongoDBPuppetConfigRepository

        client = get_mongodb_client()
        repo = MongoDBPuppetConfigRepository(client.atlas)
        return MongoDBPuppetService(repo)
    else:
        if session is None:
