(() => {
  ...
  const normaliseTheme = (value) => {
    if (value === 'atlas-light') return 'nebula-light';
    if (value === 'nebula') return 'nebula-dark';
  ...
  const applyThemeClass = (nextTheme, options = {}) => {
    ...
    if (body) {
      ...
      body.classList.remove('theme-atlas-light', 'theme-nebula');
  ...
  const initialTheme = (() => {
    ...
    if (stored === 'atlas-light') stored = 'nebula-light';
    if (stored === 'nebula') stored = 'nebula-dark';
  ...
  document.getElementById('zbx-ack')?.addEventListener('click', async (e) => {
    ...
    try {
      ...
      const msg = prompt('Ack message (optional):', 'Acknowledged via Infrastructure Atlas');
